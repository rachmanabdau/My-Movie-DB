...
class NetowrkkApiTest {
    ...
    fun getPopularTVShow_withValidApiKey_resultSuccess() {
        runBlocking {
            // WHEN user has request token and create new session
            val movieList =
                service.retrofitService.getPopularTvShowAsync(1).await()
    ...
    fun getPopularTVShows_withInvalidApiKey_resultError401() {
        runBlocking {
            // WHEN user has request token and create new session
            val movieList =
                service.retrofitService.getPopularTvShowAsync(1, "invalidKey").await()
    ...
    fun getOnAirTVShow_withValidApiKey_resultSuccess() {
        runBlocking {
            // WHEN user has request token and create new session
            val movieList =
                service.retrofitService.getOnAirTvShowAsync(1).await()
    ...
    fun getOnAirTVShows_withInvalidApiKey_resultError401() {
        runBlocking {
            // WHEN user has request token and create new session
            val movieList =
                service.retrofitService.getOnAirTvShowAsync(1, "invalidKey").await()
